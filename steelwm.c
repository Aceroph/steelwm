...
void setup(void) {
  ...
  XChangeProperty(dpy, wmcheckwin, netatom[NetWMName], utf8string, 8,
                  PropModeReplace, (unsigned char *)"steelwm", 3);
...
void spawn(const Arg *arg) {
  ...
  if (fork() == 0) {
    ...
    die("steelwm: execvp '%s' failed:", ((char **)arg->v)[0]);
  }
}
...
int xerror(Display *dpy, XErrorEvent *ee) {
  if (ee->error_code == BadWindow ||
    ...
    return 0;
  fprintf(stderr, "steelwm: fatal error: request code=%d, error code=%d\n",
...
int xerrorstart(Display *dpy, XErrorEvent *ee) {
  die("steelwm: another window manager is already running");
  return -1;
}
...
int main(int argc, char *argv[]) {
  ...
  if (!(dpy = XOpenDisplay(NULL)))
    die("steelwm: cannot open display");
  checkotherwm();
  setup();
#ifdef __OpenBSD__
